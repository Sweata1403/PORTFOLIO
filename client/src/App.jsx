...
function profileToDraft(profile) {
  return {
    ...
    socialLinksText: (profile?.socialLinks || []).map((link) => `${link.label} | ${link.url}`).join('\n'),
    profileImageFile: null,
    resumeFile: null
  };
}
...
function CertificateCard({ certificate }) {
  return (
    <article className="detail-card">
      ...
      {certificate.credentialId ? <span className="muted-text">Credential ID: {certificate.credentialId}</span> : null}
      ...
      {certificate.credentialUrl ? (
        <div className="button-row">
          <a className="ghost-button" href={certificate.credentialUrl} target="_blank" rel="noreferrer">
            View credential
          </a>
        </div>
      ) : null}
...
function PublicLayout({ profile, contact, theme, setTheme, children }) {
  ...
  // Collapse the mobile menu whenever the route changes, so tapping a link closes it.
  useEffect(() => {
    setMenuOpen(false);
  }, [location.pathname]);
    ...
    <div className="site-shell">
      <header className={menuOpen ? 'site-header nav-open' : 'site-header'}>
        <Link className="site-brand" to="/">
          {/* Decorative: the brand name sits alongside it in text. */}
          <img className="brand-mark" src="/logo.png" alt="" width="160" height="160" />
          <span>
  ...
  return (
    <div className="page-stack">
      <div className="hero-layout">
        ...
        <aside className="surface-card profile-card">
          {profile.profileImage?.url ? (
            <img className="profile-photo" src={profile.profileImage.url} alt={profile.name || 'Profile'} />
          ) : (
      ...
      <section className="stats-grid">
        <StatCard label="Projects" value={bootstrap?.projects?.length || 0} hint="Portfolio work and builds" />
        <StatCard label="Experience" value={bootstrap?.experiences?.length || 0} hint="Roles and internships" />
        <StatCard label="Certificates" value={bootstrap?.certificates?.length || 0} hint="Verified credentials and learning" />
        <StatCard label="Visitors" value={bootstrap?.metrics?.homepageViews || 0} hint="Homepage visits tracked locally" />
      </section>
        ...
        <section className="surface-card">
          ...
          <div className="card-grid">
            {featuredProjects.length ? (
              ...
              <EmptyState title="Projects coming soon" description="Use the dashboard to add project entries." />
            )}
          </div>
        </section>
          ...
          <div className="stack-md">
            {topExperiences.map((item) => (
              <article key={item.id} className="timeline-card compact">
                <div className="card-meta">
                  <span>{formatRange(item.startDate, item.endDate, item.isCurrent)}</span>
      ...
      <section className="surface-card">
        ...
        <div className="card-grid">
          {recentCertificates.length ? (
            recentCertificates.map((certificate) => <CertificateCard key={certificate.id} certificate={certificate} />)
          ) : (
            <EmptyState
      ...
      <section className="surface-card">
        ...
        <div className="timeline-stack">
          {experiences.length ? (
            ...
            <EmptyState title="No experience yet" description="Add roles from the dashboard to populate this page." />
          )}
        </div>
      </section>
    </div>
      ...
      <section className="surface-card">
        ...
        <div className="card-grid projects-grid">
          {projects.length ? (
            projects.map((project) => (
              <article key={project.id} className="detail-card project-card">
                ...
                {(project.liveUrl || project.repoUrl) && (
                  <div className="button-row">
                    {project.liveUrl ? (
                      <a className="ghost-button" href={project.liveUrl} target="_blank" rel="noreferrer">
                        Live
                      </a>
                    ) : null}
                    {project.repoUrl ? (
            ...
            <EmptyState title="No projects yet" description="Use the admin dashboard to add projects." />
          )}
        </div>
      </section>
    </div>
  ...
  return (
    <div className="page-stack">
      <div className="content-grid contact-layout">
          ...
          <div className="stack-md">
            <div className="info-grid">
              ...
              <div>
                ...
                {contact.resume?.downloadUrl ? <a href={contact.resume.downloadUrl}>Download PDF</a> : <span>Unavailable</span>}
              </div>
            </div>
        ...
        <section className="surface-card">
          <SectionTitle label="Message" title="Start a conversation." body="Use the form and the message will land in the private dashboard." />
    ...
    <div className="auth-shell">
      <section className="surface-card auth-card">
        <SectionTitle
          ...
          title={mode === 'setup' ? 'Create the owner account.' : 'Sign into the portfolio dashboard.'}
          body={
            mode === 'setup'
        ...
        <form className="stack-md" onSubmit={handleSubmit}>
          ...
          {mode === 'setup' ? (
            <label>
              <span>Setup key</span>
              <input value={form.setupKey} onChange={(event) => setForm({ ...form, setupKey: event.target.value })} />
            </label>
          ) : null}

          <div className="button-row">
            <button className="primary-button" type="submit" disabled={submitting}>
              {submitting ? 'Please wait...' : mode === 'setup' ? 'Create owner' : 'Login'}
            </button>
            {!session.setupRequired ? (
              <button
  ...
  return (
    <div className="dashboard-shell">
      ...
      <div className="dashboard-main">
        ...
        {activeTab === 'profile' ? (
          <ProfileTab dashboard={dashboard} csrfToken={session.csrfToken} refreshDashboard={refreshDashboard} />
        ) : null}
        {activeTab === 'experience' ? (
          <ExperienceTab experiences={dashboard.experiences} csrfToken={session.csrfToken} refreshDashboard={refreshDashboard} />
        ) : null}
        {activeTab === 'projects' ? (
          <ProjectsTab projects={dashboard.projects} csrfToken={session.csrfToken} refreshDashboard={refreshDashboard} />
        ) : null}
        ...
        {activeTab === 'skills' ? (
          <SkillsTab skillGroups={dashboard.skillGroups} csrfToken={session.csrfToken} refreshDashboard={refreshDashboard} />
        ) : null}
        {activeTab === 'messages' ? (
          <MessagesTab messages={dashboard.messages} csrfToken={session.csrfToken} refreshDashboard={refreshDashboard} />
        ) : null}
      </div>
    </div>
  );
    ...
    <div className="page-stack">
      <section className="surface-card">
        ...
        <div className="stats-grid">
          <StatCard label="Homepage views" value={dashboard.metrics.homepageViews} hint="Incremented on home visits" />
          <StatCard label="Messages" value={dashboard.overview.counts.messages} hint={`${dashboard.overview.counts.unreadMessages} unread`} />
          <StatCard label="Projects" value={dashboard.overview.counts.projects} hint="Published portfolio pieces" />
          <StatCard label="Experience" value={dashboard.overview.counts.experiences} hint="Timeline entries" />
          <StatCard label="Certificates" value={dashboard.overview.counts.certificates} hint="Credential entries" />
        </div>
      </section>
      ...
      <section className="surface-card">
        ...
        <form className="stack-md" onSubmit={handleProfileSubmit}>
          ...
          <div className="field-grid">
            ...
            <label>
              <span>Title</span>
              <input value={profileDraft.title} onChange={(event) => setProfileDraft({ ...profileDraft, title: event.target.value })} />
            </label>
          </div>
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setProfileDraft({ ...profileDraft, headline: event.target.value })}
            />
          </label>
          ...
          <label>
            ...
            <textarea
              ...
              onChange={(event) => setProfileDraft({ ...profileDraft, summaryText: event.target.value })}
            />
          </label>
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setProfileDraft({ ...profileDraft, focusAreasText: event.target.value })}
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setProfileDraft({ ...profileDraft, availabilityNote: event.target.value })}
            />
          </label>
          ...
          <label>
            ...
            <textarea
              ...
              onChange={(event) => setProfileDraft({ ...profileDraft, socialLinksText: event.target.value })}
          ...
          <div className="field-grid">
            <label>
              ...
              <input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => setProfileDraft({ ...profileDraft, profileImageFile: event.target.files?.[0] || null })} />
              {dashboard.profile.profileImage ? (
                <small>Current: {dashboard.profile.profileImage.originalName}</small>
              ) : null}
            </label>
            <label>
              <span>Resume PDF</span>
              <input type="file" accept="application/pdf" onChange={(event) => setProfileDraft({ ...profileDraft, resumeFile: event.target.files?.[0] || null })} />
              {dashboard.profile.resume ? <small>Current: {dashboard.profile.resume.originalName}</small> : null}
            </label>
          </div>
        ...
        <form className="stack-md" onSubmit={handleContactSubmit}>
          ...
          <label>
            ...
            <textarea
              ...
              onChange={(event) => setContactDraft({ ...contactDraft, headline: event.target.value })}
            />
          </label>
          ...
          <div className="field-grid">
            ...
            <label>
              <span>Phone</span>
              <input value={contactDraft.phone} onChange={(event) => setContactDraft({ ...contactDraft, phone: event.target.value })} />
            </label>
          </div>

          <label>
            <span>Location</span>
            <input value={contactDraft.location} onChange={(event) => setContactDraft({ ...contactDraft, location: event.target.value })} />
          </label>
...
function ExperienceTab({ experiences, csrfToken, refreshDashboard }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await refreshDashboard();
      setStatus({ tone: 'success', text: selectedId === 'new' ? 'Experience created.' : 'Experience updated.' });
    } catch (error) {
      setStatus({ tone: 'error', text: error.message });
    } finally {
      setSaving(false);
    ...
    <div className="editor-shell">
      ...
      <section className="surface-card editor-panel">
        <SectionTitle
          ...
          title={selectedId === 'new' ? 'Add a new experience entry.' : 'Update the selected experience.'}
        />
        ...
        <form className="stack-md" onSubmit={handleSubmit}>
          ...
          <div className="field-grid">
            <label>
              ...
              <input type="month" value={draft.startDate} onChange={(event) => setDraft({ ...draft, startDate: event.target.value })} />
            </label>
            <label>
              <span>End date</span>
              <input
          ...
          <label className="checkbox-row">
            <input
              ...
              onChange={(event) => setDraft({ ...draft, isCurrent: event.target.checked, endDate: event.target.checked ? '' : draft.endDate })}
            />
          ...
          <div className="button-row">
            <button className="primary-button" type="submit" disabled={saving}>
              {saving ? 'Saving...' : selectedId === 'new' ? 'Create entry' : 'Save changes'}
            </button>
            {selectedId !== 'new' ? (
              <button className="ghost-button danger" type="button" onClick={handleDelete} disabled={saving}>
                Delete
              </button>
            ) : null}
          </div>
...
function ProjectsTab({ projects, csrfToken, refreshDashboard }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await refreshDashboard();
      setStatus({ tone: 'success', text: selectedId === 'new' ? 'Project created.' : 'Project updated.' });
    } catch (error) {
      setStatus({ tone: 'error', text: error.message });
    } finally {
      setSaving(false);
  ...
  return (
    <div className="editor-shell">
      ...
      <section className="surface-card editor-panel">
        <SectionTitle
          label={selectedId === 'new' ? 'Create' : 'Edit'}
          title={selectedId === 'new' ? 'Add a new project.' : 'Update the selected project.'}
        />
        ...
        <form className="stack-md" onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <textarea rows="7" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} />
          </label>
          ...
          <label>
            ...
            <input
              ...
              onChange={(event) => setDraft({ ...draft, techStackText: event.target.value })}
          ...
          <div className="field-grid">
            <label>
              <span>Live URL</span>
              <input value={draft.liveUrl} onChange={(event) => setDraft({ ...draft, liveUrl: event.target.value })} />
            </label>
            <label>
              <span>Repository URL</span>
              <input value={draft.repoUrl} onChange={(event) => setDraft({ ...draft, repoUrl: event.target.value })} />
            </label>
          </div>
          ...
          <div className="button-row">
            <button className="primary-button" type="submit" disabled={saving}>
              {saving ? 'Saving...' : selectedId === 'new' ? 'Create project' : 'Save changes'}
            </button>
            {selectedId !== 'new' ? (
              <button className="ghost-button danger" type="button" onClick={handleDelete} disabled={saving}>
                Delete
...
function CertificatesTab({ certificates, csrfToken, refreshDashboard }) {
  ...
  useEffect(() => {
    ...
    const item = certificates.find((certificate) => certificate.id === selectedId);
    if (item) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await refreshDashboard();
      setStatus({ tone: 'success', text: selectedId === 'new' ? 'Certificate created.' : 'Certificate updated.' });
    } catch (error) {
      setStatus({ tone: 'error', text: error.message });
    } finally {
      setSaving(false);
  ...
  return (
    <div className="editor-shell">
      <section className="surface-card editor-list">
        <div className="stack-md">
          <SectionTitle label="Certificates" title="Manage credentials and learning milestones" />
          <button className="primary-button" type="button" onClick={() => setSelectedId('new')}>
            New certificate
          </button>
      ...
      <section className="surface-card editor-panel">
        <SectionTitle
          label={selectedId === 'new' ? 'Create' : 'Edit'}
          title={selectedId === 'new' ? 'Add a new certificate.' : 'Update the selected certificate.'}
        />
        ...
        <form className="stack-md" onSubmit={handleSubmit}>
          ...
          <div className="field-grid">
            <label>
              ...
              <input type="month" value={draft.issueDate} onChange={(event) => setDraft({ ...draft, issueDate: event.target.value })} />
            </label>
            <label>
              <span>Sort order</span>
              <input
          ...
          <div className="field-grid">
            <label>
              ...
              <input
                ...
                onChange={(event) => setDraft({ ...draft, credentialId: event.target.value })}
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => setDraft({ ...draft, credentialUrl: event.target.value })}
          ...
          <div className="button-row">
            <button className="primary-button" type="submit" disabled={saving}>
              {saving ? 'Saving...' : selectedId === 'new' ? 'Create certificate' : 'Save changes'}
            </button>
            {selectedId !== 'new' ? (
              <button className="ghost-button danger" type="button" onClick={handleDelete} disabled={saving}>
                Delete
...
function SkillsTab({ skillGroups, csrfToken, refreshDashboard }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await refreshDashboard();
      setStatus({ tone: 'success', text: selectedId === 'new' ? 'Skill group created.' : 'Skill group updated.' });
    } catch (error) {
      setStatus({ tone: 'error', text: error.message });
    } finally {
      setSaving(false);
  ...
  return (
    <div className="editor-shell">
      <section className="surface-card editor-list">
        <div className="stack-md">
          <SectionTitle label="Skills" title="Organize capabilities into groups" />
          <button className="primary-button" type="button" onClick={() => setSelectedId('new')}>
            New skill group
          </button>
      ...
      <section className="surface-card editor-panel">
        <SectionTitle
          label={selectedId === 'new' ? 'Create' : 'Edit'}
          title={selectedId === 'new' ? 'Add a new skill group.' : 'Update the selected skill group.'}
        />
        ...
        <form className="stack-md" onSubmit={handleSubmit}>
          ...
          <div className="button-row">
            <button className="primary-button" type="submit" disabled={saving}>
              {saving ? 'Saving...' : selectedId === 'new' ? 'Create skill group' : 'Save changes'}
            </button>
            {selectedId !== 'new' ? (
              <button className="ghost-button danger" type="button" onClick={handleDelete} disabled={saving}>
                Delete
          ...
          {messages.length ? (
            messages.map((message) => (
              <article key={message.id} className="detail-card">
                ...
                <strong>{message.email}</strong>
                {message.company ? <span className="muted-text">{message.company}</span> : null}
                <p>{message.message}</p>
                <div className="button-row">
                  <button className="ghost-button" type="button" onClick={() => toggleRead(message)} disabled={loadingId === message.id}>
                    {message.isRead ? 'Mark unread' : 'Mark read'}
                  </button>
                  <button className="ghost-button danger" type="button" onClick={() => deleteMessage(message)} disabled={loadingId === message.id}>
                    Delete
                  </button>
                </div>
              </article>
            ...
            <EmptyState title="Inbox is clear" description="Public contact form submissions will appear here." />
          )}
        </div>
      </section>
    </div>
  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      document.querySelectorAll(SELECTORS).forEach((el) => {
        if (el.classList.contains('visible') || el.classList.contains('will-reveal')) {
          return;
        }
        ...
        if (rect.top > viewH * 0.92) {
          const siblings = el.parentElement
            ? Array.from(el.parentElement.children).filter((child) => child.matches(SELECTORS))
            : [];
          const idx = siblings.indexOf(el);
          if (idx > 0) {
            el.setAttribute('data-delay', Math.min(idx, 4));
          }
    ...
    const raf = requestAnimationFrame(() => {
      attach();
      mutationObserver = new MutationObserver(() => requestAnimationFrame(attach));
...
const INTRO_WORDS = ['Senior Software Engineer', 'DevOps Engineer', 'Data Engineer', 'Cloud Architect'];
...
function IntroLoader({ onDone }) {
  ...
  useEffect(() => {
    // Per word: 400ms fade-in + 1000ms hold + 300ms fade-out, then a 700ms exit fade.
    let timer;
    let exitTimer;
  ...
  }, [phase, wordIdx, onDone]);

  return (
    <div className={exit ? 'intro-overlay intro-exit' : 'intro-overlay'}>
      <div className="intro-content">
        <div className="intro-name">Sweata Chakraborty</div>
        <div className="intro-divider" />
        <div className={`intro-word intro-${phase}`}>{INTRO_WORDS[wordIdx]}</div>
  ...
  return (
    <Routes>
      <Route
        path="/"
        element={
          <PublicLayout profile={content.profile} contact={content.contact} theme={theme} setTheme={setTheme}>
      ...
      <Route
        ...
        element={
          <PublicLayout profile={content.profile} contact={content.contact} theme={theme} setTheme={setTheme}>
            <ContactPage
              bootstrap={content}
              onSendMessage={(form) => apiRequest('/api/public/contact', { method: 'POST', body: form })}
            />
          </PublicLayout>
        }
      />
      ...
      <Route
        path="/dashboard"
        element={<DashboardPage session={session} dashboard={dashboard} onLogout={logout} refreshDashboard={refreshDashboard} />}
      />
...
export default function App() {
  ...
  const refreshBootstrap = useCallback(async (countView = false) => {
    const data = await apiRequest(`/api/public/bootstrap?countView=${countView ? 'true' : 'false'}`);
    setBootstrap(data);
    return data;
  }, []);
  ...
  return (
    <>
      <CustomCursor />
      {introComplete ? null : <IntroLoader onDone={handleIntroDone} />}
      <BrowserRouter>
        <RoutedApp
          theme={theme}
          setTheme={setTheme}
